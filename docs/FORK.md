...
- Plan stats (done, v0.5.6 + v0.5.7 hotfix) — periodic openvpn3 session.statistics fetch in the service.  Every 30 s a dedicated stats timer reads bytes_in / bytes_out / packets_in / packets_out and emits a journal line with absolute counters plus per-tick rate (bytes/s); first tick prints no rate so the seeded zeros are not mistaken for instant throughput.  Visible via `journalctl -t nm-openvpn3-service | grep stats`.  New ovpn3_session_get_statistics() in ovpn3-client.  v0.5.7 swaps the initial _LOGI emit (gated on gl.log_level >= LOG_NOTICE which is -1 by default, so the line never reached the journal) for ovpn3_trace() so the lines actually appear without NM_VPN_LOG_LEVEL being set in the env.
